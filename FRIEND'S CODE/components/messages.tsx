...
import { RefreshCw, AlertCircle, RotateCcw, Clipboard, Check } from 'lucide-react';
...
const showCenterSuccess = (message: string) => {
  ...
  setTimeout(() => {
      ...
      // Use the transitionend event to ensure animation is complete
          ...
          <div key={`${messageIndex}-${partIndex}-text`}>
            <div className="flex items-center mt-5 mb-2">
              <div className="flex items-center gap-2">
                {/* Removed Scira AI logo and heading */}
              </div>
            </div>
            <MarkdownRenderer content={preprocessLaTeX(part.text)} />
            
            {/* Regenerate and copy buttons moved below the text */}
            {status === 'ready' && (
