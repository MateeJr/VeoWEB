...
import { FileText, ArrowRight, ChevronLeft, ChevronRight, Copy, Download, X, Edit, Clipboard } from 'lucide-react';
  ...
  if (message.role === 'user') {
    return (
      <motion.div
        ...
        className="mb-4 px-0 flex justify-end"
      >
        <div className="grow min-w-0 text-right">
          {isEditingMessage && editingMessageIndex === index ? (
            <form onSubmit={handleMessageUpdate} className="w-full text-right">
                ...
                {!isEditingMessage && index === lastUserMessageIndex && (
                  <div className="flex items-center justify-end gap-2 mt-4">
                    <Button
                      ...
                      className="h-8 w-8 rounded-full bg-neutral-100 hover:bg-neutral-200 dark:bg-neutral-800 dark:hover:bg-neutral-700 text-neutral-700 dark:text-neutral-300"
                      disabled={status === 'submitted' || status === 'streaming'}
                    >
      ...
      <>
        ...
        {suggestedQuestions.length > 0 && (
          <motion.div
            ...
            exit={{ opacity: 0, y: 20 }}
            transition={{ duration: 0.5 }}
            className="w-full max-w-xl sm:max-w-2xl mt-6 ml-auto"
          >
            ...
            <div className="space-y-2 flex flex-col items-end">
              {suggestedQuestions.map((question, i) => (
                <Button
                  key={i}
                  variant="ghost"
                  className="w-fit font-medium rounded-md p-1 justify-start text-left h-auto py-2 px-4 bg-neutral-800 dark:bg-neutral-900 text-white dark:text-neutral-100 hover:bg-neutral-700 dark:hover:bg-neutral-800 whitespace-normal cursor-pointer"
